...
# going to try new model XGBoost for the prediction of SOC


# Directory where raw data is stored
raw_data_path = glob.glob("C:/Users/jmani/Documents/BatteryMLProject/src/data/complete_dataset/raw_data/*.csv")
df = {f: pd.read_csv(f) for f in raw_data_path}
...
class PreModelDataProcessor:
    ...
    def fit_global_scalers(df):
        ...
        for df in df.values():
            ...
            all_time_values.append((df['Time'] - df['Time'].min()).dt.total_seconds().values)
        ...
        time_scaler = MinMaxScaler((0, 1)).fit(all_time_values)
        soc_scaler = MinMaxScaler((0, 1)).fit(all_soc_values)
    ...
    def preprocess_data(df, time_scaler, soc_scaler):
        ...
        return np.hstack((scaled_time, scaled_soc))  # Now contains both time & SOC
    ...
    def prepare_xgboost_data(scaled_data, time_step=60):
        ...
        for i in range(time_step, len(scaled_data)):
            X.append(scaled_data[i - time_step:i, :].flatten())  # Use past time & SOC
...
def main():
    ...
    # Fit global scalers
    time_scaler, soc_scaler = PreModelDataProcessor.fit_global_scalers(df)

    # Test if scalers have been fitted
